...
--------------------------------------------------------------------
-- STEP 8: Chart 2 — why does DF break the revenue/shipping pattern,
-- and what's the real relationship between orders and revenue?
--------------------------------------------------------------------
 
-- 8.1: is DF's freight genuinely lower than similar-revenue states?
SELECT
    c.customer_state,
    ROUND(AVG(i.freight_value)::numeric, 2) AS avg_freight_per_item
FROM View_Q2 o
JOIN customers c ON o.customer_id = c.customer_id
JOIN order_items i ON o.order_id = i.order_id
WHERE c.customer_state IN ('DF', 'GO', 'ES', 'BA')
GROUP BY c.customer_state;
-- Result: DF = 21.07, ES = 22.03, GO = 22.56, BA = 26.49.
-- DF really does have the lowest average freight per item among these
-- four states — not a rounding artifact. Likely explanation: DF is
-- Brazil's capital and may have better logistics infrastructure than
-- other mid-revenue states despite not being the main economic hub.
 
-- 8.2: how tightly do order count and revenue actually move together?
SELECT
    c.customer_state,
    COUNT(DISTINCT o.order_id) AS total_orders,
    CAST(SUM(i.price) AS DECIMAL(10,2)) AS total_revenue,
    ROUND(SUM(i.price) / COUNT(DISTINCT o.order_id), 2) AS avg_revenue_per_order
FROM View_Q2 o
JOIN customers c ON o.customer_id = c.customer_id
JOIN order_items i ON o.order_id = i.order_id
GROUP BY c.customer_state
...
-- Result: ranking by total_orders and ranking by total_revenue match
-- exactly across all 12 states — a near-perfect relationship. But
-- avg_revenue_per_order isn't flat: SP sits lowest (~125), while CE
-- sits highest (~172), almost 40% more per order despite having far
-- fewer orders. States with fewer orders (CE, PE) tend to have a
-- higher basket value per order — possibly customers there bundling
-- purchases to offset the higher shipping cost per order.
